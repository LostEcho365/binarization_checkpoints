...
class CifarResNet(nn.Module):
  ...
  """
    ...
    """
        ...
        #     depth, layer_blocks))
                # Adjust the assert statement to include depth 18

        valid_depths = [18, 20, 32, 44, 56, 110]
        assert depth in valid_depths, 'depth should be one of {}'.format(valid_depths)
        if depth == 18:
            layer_blocks = [2, 2, 2, 2]
        else:
            layer_blocks = [(depth - 2) // 6] * 3
        ...
        self.stage_1 = self._make_layer(block, 16, layer_blocks[0], 1)
        self.stage_2 = self._make_layer(block, 32, layer_blocks[1], 2)
        self.stage_3 = self._make_layer(block, 64, layer_blocks[2], 2)
        if depth == 18:
            self.stage_4 = self._make_layer(block, 128, layer_blocks[3], 2)
            
        self.avgpool = nn.AvgPool2d(4)
        self.classifier = quan_Linear(128 * block.expansion, num_classes)
